...
export class BugDetectionAgent extends BaseAgent {
  ...
  private checkCommonBugs(content: string, filePath: string): CodeIssue[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isComment = line.startsWith('//') || line.startsWith('#') || line.startsWith('/*') || line.startsWith('*') || line.startsWith('<!--');
      
      if (isComment && line.length > 3) {
        // Check if the comment contains code-like patterns
        const containsCode = /[{};=><\[\]()]/.test(line) || /\bif\b|\bfor\b|\bwhile\b|\bfunction\b|\bclass\b|\bdef\b|\breturn\b/.test(line);
      ...
      if (/\w+\.\w+/.test(line) && !line.includes('?') && !line.includes('&&') && !line.includes('||')) {
        ...
        if (matches) {
          for (const match of matches) {
            ...
            if (!['this', 'self', 'window', 'document', 'console', 'process', 'module', 'require', 'exports'].includes(obj)) {
              issues.push(this.createIssue(
  ...
  private checkPythonBugs(content: string, filePath: string): CodeIssue[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/except\s*:/.test(line) && !line.includes('Exception') && !line.includes('BaseException')) {
        issues.push(this.createIssue(
          'Bare except clause',
  ...
  private checkJavaBugs(content: string, filePath: string): CodeIssue[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/\w+\.\w+/.test(line) && !line.includes('!=') && !line.includes('==') && !line.includes('null')) {
        const matches = line.match(/(\w+)\.\w+/g);
        if (matches) {
          for (const match of matches) {
            const obj = match.split('.')[0];
            if (!['this', 'super', 'System', 'Math', 'String', 'Integer', 'Boolean', 'Double', 'Float', 'Long'].includes(obj)) {
              issues.push(this.createIssue(
      ...
      if (/catch\s*\([^)]+\)\s*\{\s*\}/.test(line) || (line.includes('catch') && i + 1 < lines.length && lines[i + 1].trim() === '}')) {
        issues.push(this.createIssue(
          'Empty catch block',
